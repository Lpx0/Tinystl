...
namespace TinySTL {
	...
	namespace VectorTest {
		...
		void testCase1() {
			...
			tsVec<TinySTL::string> v4(10);
			assert(TinySTL::Test::container_equal(v3, v4));

			std::array<TinySTL::string, 3> arr = { "abc", "def", "ghi" };
			stdVec<TinySTL::string> v5(std::begin(arr), std::end(arr));
			tsVec<TinySTL::string> v6(std::begin(arr), std::end(arr));
		...
		void testCase11() {
			stdVec<TinySTL::string> v1;
			tsVec<TinySTL::string> v2;
