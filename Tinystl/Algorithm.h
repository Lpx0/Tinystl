...
namespace TinySTL {
	...
	void make_heap(RandomAccessIterator first, RandomAccessIterator last) {
		TinySTL::make_heap(first, last, typename TinySTL::less<TinySTL::iterator_traits<RandomAccessIterator>::value_type>());
	}
	...
	void push_heap(RandomAccessIterator first, RandomAccessIterator last) {
		TinySTL::push_heap(first, last, TinySTL::less<typename TinySTL::iterator_traits<RandomAccessIterator>::value_type>());
	}
	...
	bool is_heap(RandomAccessIterator first, RandomAccessIterator last) {
		return TinySTL::is_heap(first, last, TinySTL::less<typename TinySTL::iterator_traits<RandomAccessIterator>::value_type>());
	}
	...
	bool is_heap(RandomAccessIterator first, RandomAccessIterator last, Compare comp) {
		...
		for (auto cur = first + range / 2 - 1; cur >= first; --cur, --index) {
			if(*(first + (index * 2 + 1)) > *cur || (first + (index * 2 + 2) <= last && *(first + (index * 2 + 2)) > *cur))
				return false;
